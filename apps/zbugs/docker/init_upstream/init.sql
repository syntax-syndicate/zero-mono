...
CREATE TABLE "users" (
    ...
    "name" VARCHAR,
    "avatar" VARCHAR,
    "role" VARCHAR DEFAULT 'users' NOT NULL,
    "githubID" INTEGER NOT NULL
);

CREATE UNIQUE INDEX user_login_idx ON "users" (login);
CREATE UNIQUE INDEX user_githubid_idx ON "users" ("githubID");
...
CREATE TABLE issues (
    ...
    "creatorID" VARCHAR REFERENCES "users"(user_id) NOT NULL,
    "assigneeID" VARCHAR REFERENCES "users"(user_id),
...
CREATE TABLE "viewState" (
    "userID" VARCHAR REFERENCES "users"(user_id) ON DELETE CASCADE,
    "issueID" VARCHAR REFERENCES issues(issue_id) ON DELETE CASCADE,
...
CREATE TABLE comment (
    ...
    "body" TEXT NOT NULL,
    "creatorID" VARCHAR REFERENCES "users"(user_id)
);
...
CREATE TABLE emoji (
    ...
    "creatorID" VARCHAR REFERENCES "users"(user_id) ON DELETE CASCADE,
    "created" double precision DEFAULT (EXTRACT(EPOCH FROM CURRENT_TIMESTAMP) * 1000),
...
RETURNS TRIGGER AS $$
BEGIN
    -- Check if subjectID exists in the issues table
    IF EXISTS (SELECT 1 FROM issues WHERE issue_id = NEW."subjectID") THEN
        NULL; -- Do nothing
    ELSIF EXISTS (SELECT 1 FROM comment WHERE id = NEW."subjectID") THEN
        NULL; -- Do nothing
    ELSE
        RAISE EXCEPTION 'id ''%'' does not exist in issues or comment', NEW."subjectID";
    END IF;
...
-- Delete emoji when issues is deleted
...
-- When an emoji is added or deleted we find the issues and update the modified time
...
BEGIN
    ...
    SET modified = EXTRACT(EPOCH FROM CURRENT_TIMESTAMP) * 1000
    FROM (
        SELECT issues.issue_id AS id
        FROM issues JOIN comment ON issues.issue_id=comment."issueID"
        WHERE comment.id = "subjectID" OR issues.issue_id = "subjectID"
    ) AS subquery
    WHERE issues.issue_id = subquery.id;
END;   
$$ LANGUAGE plpgsql;
...
COPY "issues" ("issue_id","shortID","title","open","modified","created","creatorID","assigneeID","description")
FROM
...
CREATE INDEX issue_modified_idx ON issues (modified);

CREATE INDEX issue_created_idx ON issues (created);
